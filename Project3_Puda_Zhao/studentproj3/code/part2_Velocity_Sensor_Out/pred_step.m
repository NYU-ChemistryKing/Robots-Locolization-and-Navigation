...
%% BEFORE RUNNING THE CODE CHANGE NAME TO pred_step
    ...
    k = 3;
    g = [-0.23; -0.2; -9.829];
    QQ = diag([ones(1,6)*1e-4, ones(1,6)*1e-4]); % process noise cov (12*12) [ng;na;nbg;nba]
    noise = 1e-2 * randn(12,1);
    n = length(uPrev) + length(noise);

    % R and G 
    % Z-Y-X Euler Angle with q = [roll_y, pitch_y, yaw_x]
